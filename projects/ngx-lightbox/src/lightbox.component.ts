    ...
    >
      <div class="lb-container" #container id="container">
        ...
        <div class="lb-nav" [hidden]="!ui.showArrowNav" #navArrow>
          <a
            class="lb-prev"
            [hidden]="!ui.showLeftArrow"
            (click)="prevImage()"
            #leftArrow
          ></a>
          <a
            class="lb-next"
            [hidden]="!ui.showRightArrow"
            (click)="nextImage()"
            #rightArrow
          ></a>
        </div>
    ...
    <div class="lb-dataContainer" [hidden]="ui.showReloader" #dataContainer>
      <div class="lb-data">
        <div class="lb-details">
          ...
          <span
            class="lb-number animation fadeIn"
            [hidden]="!ui.showPageNumber"
            #number
...
export class LightboxComponent
  implements OnInit, AfterViewInit, OnDestroy, OnInit
{
  ...
  constructor(
    ...
    this._event.subscription = this._lightboxEvent.lightboxEvent$.subscribe(
  ...
  ngOnInit(): void {
    this.album.forEach((album) => {
      if (album.caption) {
        album.caption = this._sanitizer.sanitize(
  ...
  public ngAfterViewInit(): void {
    ...
    this._cssValue = {
      containerTopPadding: Math.round(
        this._getCssStyleValue(this._containerElem, 'padding-top')
      ),
      containerRightPadding: Math.round(
        this._getCssStyleValue(this._containerElem, 'padding-right')
      ),
      containerBottomPadding: Math.round(
        this._getCssStyleValue(this._containerElem, 'padding-bottom')
      ),
      containerLeftPadding: Math.round(
        this._getCssStyleValue(this._containerElem, 'padding-left')
      ),
      imageBorderWidthTop: Math.round(
        this._getCssStyleValue(this._imageElem, 'border-top-width')
      ),
      imageBorderWidthBottom: Math.round(
        this._getCssStyleValue(this._imageElem, 'border-bottom-width')
      ),
      imageBorderWidthLeft: Math.round(
        this._getCssStyleValue(this._imageElem, 'border-left-width')
      ),
      imageBorderWidthRight: Math.round(
        this._getCssStyleValue(this._imageElem, 'border-right-width')
      ),
    };
  ...
  public control($event: any): void {
    ...
    if ($event.target.classList.contains('lb-turnLeft')) {
      ...
      ).style.webkitTransform = `rotate(${this.rotate}deg)`;
      this._lightboxEvent.broadcastLightboxEvent({
      ...
      height =
        parseInt(document.getElementById('outerContainer').style.height, 10) /
        1.5;
      width =
        parseInt(document.getElementById('outerContainer').style.width, 10) /
        1.5;
      document.getElementById('outerContainer').style.height = height + 'px';
      document.getElementById('outerContainer').style.width = width + 'px';
      height =
        parseInt(document.getElementById('image').style.height, 10) / 1.5;
  ...
  private _rotateContainer(): void {
    ...
    if ((temp / 90) % 4 === 1 || (temp / 90) % 4 === 3) {
      document.getElementById('outerContainer').style.height =
        document.getElementById('image').style.width;
      document.getElementById('outerContainer').style.width =
  ...
  private _calcTransformPoint(): void {
    const height = parseInt(document.getElementById('image').style.height, 10);
    const width = parseInt(document.getElementById('image').style.width, 10);
  ...
  private _onLoadImageSuccess(): void {
    ...
    imageHeight = naturalImageHeight =
      this._imageElem.nativeElement.naturalHeight;
    if (this.options.fitImageInViewPort) {
      ...
      maxImageHeight =
        ...
        120;
      if (
        naturalImageWidth > maxImageWidth ||
        naturalImageHeight > maxImageHeight
      ) {
        if (
          naturalImageWidth / maxImageWidth >
          naturalImageHeight / maxImageHeight
          ...
          imageHeight = Math.round(
            naturalImageHeight / (naturalImageWidth / imageWidth)
          );
        } else {
          imageHeight = maxImageHeight;
          imageWidth = Math.round(
            naturalImageWidth / (naturalImageHeight / imageHeight)
          );
        }
      }

      this._rendererRef.setStyle(
  ...
  private _centerVertically(imageWidth: number, imageHeight: number): void {
    ...
    this._rendererRef.setStyle(
      this._lightboxElem.nativeElement,
      'top',
      `${topDistance}px`
    );
  }
  ...
  private _postResize(newWidth: number, newHeight: number): void {
    ...
    this._rendererRef.setStyle(
      this._dataContainerElem.nativeElement,
      'width',
      `${newWidth}px`
    );
    this._showImage();
  ...
  private _positionLightBox(): void {
    ...
    const top =
      (this._windowRef.pageYOffset ||
        this._documentRef.documentElement.scrollTop) +
      this.options.positionFromTop;
    const left =
      this._windowRef.pageXOffset ||
      this._documentRef.documentElement.scrollLeft;
    ...
    if (this.options.disableScrolling) {
      this._rendererRef.addClass(
        this._documentRef.documentElement,
  ...
  private _addCssAnimation(): void {
    ...
    this._rendererRef.setStyle(
      this._captionElem.nativeElement,
      '-webkit-animation-duration',
      `${fadeDuration}s`
    );
    this._rendererRef.setStyle(
    ...
    this._rendererRef.setStyle(
      this._numberElem.nativeElement,
      '-webkit-animation-duration',
      `${fadeDuration}s`
    );
    this._rendererRef.setStyle(
  ...
  private _albumLabel(): string {
    ...
    return this.options.albumLabel
      .replace(/%1/g, Number(this.currentImageIndex + 1))
    ...
    if (this.album.length > 1) {
      if (this.options.wrapAround) {
        if (alwaysShowNav) {
          ...
          this._rendererRef.setStyle(
            this._leftArrowElem.nativeElement,
            'opacity',
            '1'
          );
          this._rendererRef.setStyle(
            this._rightArrowElem.nativeElement,
            'opacity',
            '1'
          );
        }
  ...
  private _enableKeyboardNav(): void {
    this._event.keyup = this._rendererRef.listen(
      'document',
      'keyup',
      (event: any) => {
